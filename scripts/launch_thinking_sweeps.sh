...
MODELS="qwen3-8b-think"
CONDITIONS="flip placebo neg_own neg_other"

echo "=== MedQA / LogiQA 2.0 / AGIEval: fill missing cells + cue experiments, --max-new-tokens 8192 ==="
for model in $MODELS; do
    for dataset in medqa logiqa2 agieval; do
        sbatch scripts/sweep_model_custom.sh "$model" "$dataset" 8192 $CONDITIONS
    done
done
